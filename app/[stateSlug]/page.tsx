...
import {
  DEFAULT_MED_SPA_CARE_TYPES_SENTENCE,
  medSpaCategorySchemaThings,
...
export async function generateMetadata({
  ...
  const title = `Medical Spas in ${stateName} | ${totalFacilities.toLocaleString()} Verified Listings | MedicalSpaDirectories.com`;

  const descriptor = `Browse ${totalFacilities.toLocaleString()} verified medical spa listings across ${cities.length.toLocaleString()} ${stateName} cities. Compare aesthetic treatments, laser services, injectables, and skin care — all rated 3 stars or higher on Google Maps.`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${stateName} medical spa directory preview`,
        },
      ],
    },
  };
...
export default async function StatePage({ params }: StatePageProps) {
  ...
  const medSpaFocusText =
    "medical spas, medi spas, aesthetic clinics, laser treatments, injectables, and skin care";
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: `How many medical spas are in ${stateName}?`,
        acceptedAnswer: {
          "@type": "Answer",
      ...
      {
        "@type": "Question",
        name: `What types of aesthetic and medical spa services are available in ${stateName}?`,
        acceptedAnswer: {
          "@type": "Answer",
  ...
  const webpageSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `Medical Spas in ${stateName}`,
    ...
    about: [
      {
        "@type": "Thing",
        name: `${stateName} medical spa listings`,
      },
      ...medSpaCategorySchemaThings(),
    ],
    speakable: {
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section className="rounded-2xl bg-surface-muted px-5 py-6 text-foreground shadow-lg shadow-navy/10 ring-1 ring-gold/40 sm:px-8 sm:py-8">
        ...
        <h1 className="mt-2 text-3xl font-semibold sm:text-4xl">
          Medical Spas in {stateName}
        </h1>
        <p className="mt-3 max-w-2xl text-sm text-foreground/80">
          Explore {medSpaFocusText} across {stateName}, including major city
          ...
          >
            American Med Spa Association resources
          </a>{" "}
          for industry trends, licensing context, and how to choose a medical spa or aesthetic provider.
        </p>
        ...
        return (
          <section className="mt-8 rounded-2xl border-2 border-teal/20 bg-teal/5 p-6 space-y-4">
            ...
            <p className="text-sm text-slate-600">
              Featured medical spas in {stateName} — verified listings with priority placement.
            </p>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-navy border-b-2 border-teal/50 pb-1 inline-block">
              Medical Spas by City in {stateName}
            </h2>
            <p className="mt-1 max-w-2xl text-sm text-slate-600">
              Choose a city to browse medical spas, medi spas, and aesthetic clinics in{" "}
              {stateName}, including laser treatments, injectables, and skin care services.
            </p>
          </div>
